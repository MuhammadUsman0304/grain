				...
				<div class="card shadow-lg text-dark">
					...
					<div class="card-body text-center">
						<table class="table table-responsive table-hover table-bordared">
							...
							<tbody>
								<?php 
								 $query = "SELECT * FROM `newslatter_info`";
								 $result = mysqli_query($con, $query);
								 if ($row = mysqli_num_rows($result)>0) {
								 	while($data = mysqli_fetch_assoc($result)){
								?>
								<tr>
									<td><?php echo htmlentities($data['newsl_id']); ?></td>
									...
									<td><a href="handler/delete_info_qry.php?qid=<?php echo htmlentities($data['newsl_id']); ?>"><i class="fa fa-trash"></i></a></td>
								</tr>
								<?php
								 	}
								 }else{
